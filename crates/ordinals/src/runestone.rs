...
impl Runestone {
  ...
  pub fn decipher_from_scriptpubkey(scriptpubkey: &str, out_len: usize) -> Option<Artifact> {
    let payload = match Runestone::payload_from_scriptpubkey(scriptpubkey) {
    ...
    let Message {
      ...
      edicts,
      mut fields,
    } = Message::from_integers_by_out_len(out_len, &integers);
    ...
    let pointer = Tag::Pointer.take(&mut fields, |[pointer]| {
      ...
      (u64::from(pointer) < u64::try_from(out_len).unwrap()).then_some(pointer)
    });
  ...
  fn payload_from_scriptpubkey(scriptpubkey: &str) -> Option<Payload> {
    let sb = ScriptBuf::from_hex(scriptpubkey).unwrap();
    let mut instructions = sb.instructions();
...
mod tests {
  ...
  #[test]
  fn test_decode_op_return() {
    let sb = ScriptBuf::from_hex("6a5d0714f2823414a802").unwrap();
    let mut instructions = sb.instructions();
    let mut payload = Vec::new();
    ...
    let p = Some(Payload::Valid(payload));
